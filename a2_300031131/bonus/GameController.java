...
public class GameController implements ActionListener {
    ...
    // ADD YOU CODE HERE
        if(e.getSource() instanceof DotButton && flag==0){
            DotButton src= (DotButton) e.getSource();
            int x=src.getColumn();
            int y=src.getRow();
            if(!gameModel.get(x,y).isCovered() && !gameModel.get(x,y).isFlag()){
                gameModel.uncover(x,y);
                gameModel.step();
            }
            gameModel.get(x,y).click();
            if(gameModel.isBlank(x,y) && !gameModel.isMined(x,y) && !gameModel.get(x,y).isFlag()){
                clearZone(gameModel.get(x,y));
            }
            gameView.update();
        }
        if(e.getSource() instanceof DotButton && flag==1){
            DotButton src= (DotButton) e.getSource();
            int x=src.getColumn();
            int y=src.getRow();
            if(!gameModel.get(x,y).isCovered() && !gameModel.get(x,y).isFlag() &&gameModel.getFlag()!=0){
                gameModel.get(x,y).setFlag();
                gameModel.decrementFlag();
                gameModel.step();
            }
            else if(gameModel.get(x,y).isFlag()){
                gameModel.incrementFlag();
                gameModel.step();
                gameModel.get(x,y).setFlag();
            }
            gameView.update();
        ...
        while(!stack.isEmpty()){
            ...
            for(int i=xmin;i<xmax+1;i++){
                for(int j=ymin;j<ymax+1;j++){
                    if(!gameModel.isCovered(i,j) && !gameModel.get(i,j).isFlag()){
                        gameModel.uncover(i,j);
